...
>>>>>>>> upstream/htmlServer:LoginForm.java
=======
import javax.swing.*;
import java.io.*;
import java.net.ServerSocket;
            ...
            while (true) {
                ...
                if (!postInfo.equals("")) {
                    ...
                    if (getRequest.split("\\s")[1].equals("/chat")) {
                        ...
                        if (Login.getCredentials(fields[0].split("=")[1], fields[1].split("=")[1]).equalsIgnoreCase(
                                "success") && getRequest.toString().split("\\s")[1].equals("/login")) {
                            // ChatClient chatClient = new ChatClient("localhost", 3000, pNumber);
                            dos.writeUTF(fields[0].split("=")[1]);
                            outputLine = "HTTP/1.1 200 OK\n" + "Content-Type: text/html" + "\n\n" + htmlChat;
                            userNum = fields[0].split("=")[1];
                            // System.out.println(userNum);
                        } else if (Login.getCredentials(fields[0].split("=")[1], fields[1].split("=")[1])
                                .equalsIgnoreCase("invalid pin")
                                && getRequest.toString().split("\\s")[1].equals("/login")) {
                            outputLine = "HTTP/1.1 200 OK\n" + "Content-Type: text/html" + "\n\n" + htmlFormPin;
                        } else {
                            outputLine = "HTTP/1.1 200 OK\n" + "Content-Type: text/html" + "\n\n" + htmlFormNumber;
                        }
